...
export async function addComment(comment, userId, postId) {
  ...
  try {
    ...
    // const { success, sentiment } = await getSentiment(comment);
    // if (success) {
    //   commentInstance.sentiment = sentiment?.toUpperCase();
    // } else {
    //   console.log("Failed to fetch sentiment for comment, defaulting to Neutral");
    //   commentInstance.sentiment = "NEUTRAL";
    // }
  ...
  try {
    ...
    comments.forEach((comment) => {
      const commentResponse = {
        ...
        // sentiment: comment.sentiment, // Include sentiment in the response
      };
